...
resource "google_compute_firewall" "allow_ssh" {
  name    = "${var.name}-cluster-allow-ssh"
  ...
  target_tags   = ["${var.name}-cluster-node"]
}

resource "google_compute_firewall" "allow_internal" {
  name    = "${var.name}-cluster-allow-internal"
  network = google_compute_network.vpc.name
  allow {
    protocol = "icmp"
  }
...
resource "google_compute_instance" "compute_nodes" {
  ...
  metadata_startup_script = <<-BOOT_SCRIPT
    ...
    echo "flux R encode --hosts=fux-[0-255]"
    flux R encode --hosts=fux-[0-255] --local > /usr/etc/flux/system/R
    printf "\n📦 Resources\n"
