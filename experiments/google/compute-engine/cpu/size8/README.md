# "Bare Metal" on Compute Engine Size 8
...
for i in $(seq 1 5); do 
  echo "Running iteration $i"
  time flux run --setattr=user.study_id=$app-$size-iter-$i -N8 -n 384 --env OMPI_MCA_btl_vader_single_copy_mechanism=none -o cpu-affinity=per-task osu_allreduce |& tee ./results/osu-allreduce/no-caliper/log-$i.out
done
...
for i in $(seq 1 5); do 
  echo "Running iteration $i"
  export CALI_CONFIG=spot,output=/tmp/out.cali,profile.mpi,mpi.message.count,mpi.message.size
  flux run --setattr=user.study_id=$app-$size-iter-$i -N8 -n 384 --env OMPI_MCA_btl_vader_single_copy_mechanism=none -o cpu-affinity=per-task osu_allreduce |& tee ./results/osu-allreduce/no-caliper/log-$i.out
  unset CALI_CONFIG
  sleep 3
...
```bash
oras push ghcr.io/converged-computing/google-performance-study:compute-engine-cpu-8 ./results/
```
