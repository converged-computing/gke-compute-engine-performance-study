...
void Verlet::run(int n)
{
  ...
  // vsoch: start caliper loop
  CALI_CXX_MARK_LOOP_BEGIN(timestep_loop, "main_timestep_loop");
  for (int i = 0; i < n; i++) {

    // Mark the start of an iteration
    CALI_CXX_MARK_LOOP_ITERATION(timestep_loop, n);
  ...
  // Mark end of the loop
  CALI_CXX_MARK_LOOP_END(timestep_loop);
}
